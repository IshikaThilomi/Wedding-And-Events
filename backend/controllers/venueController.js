...
exports.addNewVenueType = (req, res) => {
  ...
  form.parse(req, (err, fields, file) => {
    ...
    //Destructuring the feilds
    const{ venueName, venueType, venueDescription, venueLocation,occupacy,area,features} = fields;

    //validating input fields
    if (!venueName || !venueType || !venueDescription || !venueLocation || !occupacy|| !area || !features) {
      return res.status(400).json({
